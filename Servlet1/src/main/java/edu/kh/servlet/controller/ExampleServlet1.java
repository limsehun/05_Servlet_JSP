 ...
 * -> 상속받아서 사용
 * 
 * -해당 클래스를 상속받았다고 해서 클라이언트의 요청을 받아서 처리할 수 있는 진짜 Servlet이 된 것은 아니다.
 * 
 * [Servlet 등록절차]
...
public class ExampleServlet1 extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * - 클라이언트 요청 시 생성되는 객체
		 * 
		 * - 요청시 전달된 데이터, 요청한 클라이언트의 정보, 유연한 요청 처리를 위한 객체를 제공함
		 * 
		 * HttpServletResponse
		 * - 클라이언트 요청시 생성되는 객체
		 * 
		 ...
		 * 
		 * 2) 클라이언트의 브라우저가 응답 받은 코드를 해석해 화면을 보여줌
		 */
		...
		// UTP-8   : 유니코드를 위한 가변 길이 문자 인코딩 방식
		//	- 영어, 숫자, 기본 특수문자 : 1byte
		//	- 나머지(한글, 한문 등)		: 3byte
		...
		// *응답처리 4* : 출력 스트림을 이용해 클라이언트에게 HTML 코드 출력(응답)
